...
-------------------------------
Test simulation perfromance and stability of the frankaic arm dealing with a set of complex objects in a bin.
"""
...
class CapRotatingEnv():
    def __init__(self, cfg, task_name="cap_rotating", num_points=512, save_path="log", save_images=True, save_pts=True):
    ...
    def create_env(self, cfg):
        ...
        mob_height = self.frame_transformation.r.rotate(gymapi.Vec3(0., 0.32, 0.))
        spawn_height = self.frame_transformation.r.rotate(gymapi.Vec3(0., 0.16, 0.))
        
        # franka_asset_file = "assets/franka_description/robots/franka_with_cap_collision.urdf"
        franka_asset_file = "assets/franka_description/robots/franka_with_cap.urdf"
        mobility_asset_file = "assets/mobility.urdf"
        
        franka_assets = []
        object_assets = []

        franka_assets.append(self.gym.load_asset(self.sim, asset_root, franka_asset_file, asset_options))
        object_assets.append(self.gym.load_asset(self.sim, asset_root, mobility_asset_file, asset_options))
        ...
        franka_pose.p = gymapi.Vec3(0, 0.13, 0)
        ...
        mobility_pose = gymapi.Transform()
        mobility_pose.p = table_surface_center + spawn_height
        # mobility_pose.p = gymapi.Vec3(0, 2, 0)
        mobility_pose.r = gymapi.Quat(-np.sqrt(2)/2, 0, 0, np.sqrt(2)/2) * self.frame_transformation.r
        mobility_asset = object_assets[0]
        mobility = self.gym.create_actor(env, mobility_asset, mobility_pose, "mobility" , 0, 0)
        self.gym.set_rigid_body_color(env, mobility, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1., 1., 1.))
        
        franka_hand = "cap"
        self.franka_handles.append(franka)
        # self.object_handles.append(mobility)
        ...
        attractor_properties.target.p = table_surface_center + mob_height
