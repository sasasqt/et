...
from etseed.utils.se_math import se3
from etseed.utils.data_utils import bezier_curve
...
class DiffusionScheduler(torch.nn.Module):
    def __init__(self):
        ...
        self.mode = 'squaredcos_cap_v2' # ['linear','cosine','squaredcos_cap_v2']
        self.S = 0.008 # 0.008
    ...
    def add_noise(self,
        ...
        # exit(0)
        print(H_T.shape,alpha_bars.shape)
        print((H_T @ torch.inverse(original_samples)).shape)
        print((se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32)))).shape)
        #print(tmp.shape,'1-')
        print((((1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32))))).shape)
        tmp=se3.exp((1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32))))
        print(tmp.shape,'exp')
        tmp=(1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32)))
        print(tmp.shape,'before')
        print((se3.exp((1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32))))).shape)
        #print(se3.exp((1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32)))).shape)
        H_t = se3.exp((1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32)))) @ original_samples.to(torch.float32)
        print(H_t.shape)
        #print((H_T @ torch.inverse(original_samples)).shape)
        #print((se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32)))).shape)
        #print((se3.exp(1. - torch.sqrt(alpha_bars))).shape)
        #print(se3.exp((1. - torch.sqrt(alpha_bars)) * se3.log(H_T @ (torch.inverse(original_samples).to(torch.float32)))).shape)
        # add noise
        ...
        # exit(0)
        noise = torch.sqrt(1. - alpha_bars) * scale * torch.randn(B, 6).to(device)  # [B, 6]
        ...
        return H_t_noise, H_noise # [B, 4, 4]
